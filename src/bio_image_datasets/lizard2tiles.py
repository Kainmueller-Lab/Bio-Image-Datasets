...
def center_pad_to_size(array, desired_shape=224):
    """
    Center pad an array with zeros to reach the desired shape.

    Args:
        array (numpy.ndarray): The array to pad.
        desired_shape (tuple): The desired shape after padding.
...
def transform_to_tiles(dataset, idx, tile_size=224):
    """Transform a dataset sample into center-padded tiles of specified size.

    Args:
        dataset: The dataset object from which to retrieve the sample.
        idx: The index of the sample in the dataset.
        tile_size: The desired tile size after padding (default is 224).

    Returns:
        A list of dictionaries, each containing a tile of the image,
        instance mask, semantic mask, and the sample name for that tile.
    """

    # Get the data
    image = dataset.get_he(idx)  # Image in CHW format
...
if __name__ == "__main__":
    ...
    # Transform a sample into tiles
    idx = 111
    tiles = transform_to_tiles(dataset, idx)
    ...
    tiles = transform_to_tiles(dataset, sample_idx, tile_size=224)
