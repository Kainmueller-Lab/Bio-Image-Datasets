...
class PanNukeDataset(Dataset):
    ...
    def __len__(self):
        ...
        return np.shape(self.types["fold1"])[0] + np.shape(self.types["fold2"])[0] + np.shape(self.types["fold3"])[0]
    ...
    def prepare_semantic_masks(self, masks):
        """Prepare the semantic mask.

        The masks are prepared based on the pannuke mask which contains each semantic class
    ...
    def prepare_instance_masks(self, masks):
        """Prepare the instance segmentation mask.
