...
from bio_image_datasets.lizard2tiles import center_pad_to_size, transform_to_tiles
import scipy.io as sio
from PIL import Image


class TestLizard2Tiles(unittest.TestCase):
    ...
    def test_transform_to_tiles(self):
        ...
        tiles = transform_to_tiles(self.dataset, self.sample_idx, tile_size=224)
        ...
        for tile in tiles:
            ...
            if len(unique_ids) > 1:  # If there are instances in the tile
                self.assertEqual(unique_ids.min(), 0, "Instance IDs should start from 0.")
                self.assertEqual(unique_ids.max(), len(unique_ids) - 1, "Instance IDs should be sequential.")
    ...
    def test_transform_to_tiles_edge_cases(self):
        ...
        tiles = transform_to_tiles(self.dataset, self.sample_idx, tile_size=600)
        ...
        tiles = transform_to_tiles(self.dataset, self.sample_idx, tile_size=500)
        ...
        tiles = transform_to_tiles(self.dataset, self.sample_idx, tile_size=333)
